...
with tab3:  
    ...
    league_id = st.selectbox("League", ["GB1", "ES1", "IT1", "GER1", "FR1", "Other"])
    contract_length = st.selectbox("Contract Length", ["<6m", "6-12m", "1-2y", "2-5y", "5y+"])
...
if position == "Goalkeeper":
    st.subheader("Goalkeeper-Specific Stats")
    col_gk1, col_gk2 = st.columns(2)

    with col_gk1:
        saves = st.slider("Saves", min_value=0, max_value=200, step=10, value=0)
        save_percentage = st.slider("Save Percentage", min_value=0.0, max_value=100.0, step=5.0, value=0.0)
    
    with col_gk2:
        clean_sheets = st.slider("Clean Sheets", min_value=0, max_value = 30, step=1, value=0)
        clean_sheet_percentage = st.slider("Clean Sheet Percentage", min_value=0.0, max_value=100.0, step=5.0, value=0.0)
else:
        saves = 0
        save_percentage = 0.0
        clean_sheets = 0
