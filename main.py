...
players['contract_expiration_date'] = pd.to_datetime(players['contract_expiration_date'], errors='coerce')  #converts contract expiration date to datetime, coercing errors to NaT
...
#df = pd.merge(players[['player_id', 'contract_expiration_date']], df, on='player_id', how='right')  #connects player contract expiration date to main dataframe, allowing us to calculate player age at the time of valuation, which is an important factor in player valuation and can help improve the performance of our regression model

df['months_remaining_on_contract'] = (
    (df['contract_expiration_date'].dt.year - df['valuation_year']) * 12 +
    (df['contract_expiration_date'].dt.month - df['valuation_month'])
)

df['months_remaining_on_contract'] = df['months_remaining_on_contract'].clip(upper=60, lower=0) #calculates the number of months remaining on a player's contract at the time of valuation by finding the difference between contract expiration date and valuation date, and clipping the values to a reasonable range (0 to 60 months) to handle cases where contract expiration date may be missing or far in the future, which can help improve the performance of our regression model by providing a more accurate representation of contract length as a factor in player valuation
df['months_remaining_on_contract'] = df['months_remaining_on_contract'].fillna(60) #fills missing values for months remaining on contract with 60, which assumes that if we don't have data on a player's contract expiration date, we will treat it as if the player has the maximum remaining contract length at the time of valuation, which is a reasonable assumption for handling missing data in this context

df['contract_bucket'] = pd.cut(df['months_remaining_on_contract'], bins=[-1, 6, 12, 24, 60, 999],
    labels=['<6m', '6-12m', '1-2y', '2-5y', '5y+']) #creates a new categorical feature for contract length buckets based on the number of months remaining on a player's contract at the time of valuation, which can help capture potential non-linear effects of contract length on player valuation in our regression model

contract_dummies = pd.get_dummies(df['contract_bucket'], prefix='contract')
df = pd.concat([df, contract_dummies], axis=1) #one-hot encodes contract length buckets to capture potential non-linear effects of contract length on player valuation in our regression model
...
def create_name_mapping(df, advanced_metrics_keys): #creates a mapping of player IDs to names in the advanced metrics data, using exact matches and close matches based on name similarity, to allow us to connect the advanced metrics to the main dataframe based on player ID
    name_mapping = {}
...
if id_to_advanced:
    ...
    for player_id, metrics in id_to_advanced.items():
        advanced_records.append({
            ...
            'save_percentage': metrics.get('save_percentage', 0),
            'clean_sheets': metrics.get('clean_sheets', 0),
            'clean_sheet_percentage': metrics.get('clean_sheet_percentage', 0)
                                 })
    ...
    adv_cols = ['tackles_interceptions_per_90', 'clearances_per_90', 'blocks_per_90', 
                'recoveries_per_90', 'carries_per_90', 'distance', 'expected_goal_contributions', 'saves', 'save_percentage', 'clean_sheets', 'clean_sheet_percentage'] #defines the list of advanced metric columns to check for in the main dataframe, which can help us ensure that we are handling missing values for all of the advanced metrics we want to include in our analysis and model
    for col in adv_cols:
...
features = ['goals', 'assists', 'minutes_played', 'yellow_cards', 'red_cards', 'goals_per_90', 
    ...
    'contract_2-5y',
    'contract_5y+'
     ] + league_cols #defines the features to be used in the regression model, including performance metrics, position, age, and league dummies
x = df[features]
...
print(f"Train MAE: {train_mae:.4f}") #evaluates the performance of the regression model on the training data using Mean Absolute Error, which indicates the average absolute difference between the predicted and actual values

train_mse = mean_squared_error(y_train, model.predict(X_train))
print(f"Train MSE: {train_mse:.4f}") #evaluates the performance of the regression model on the training data using Mean Squared Error, which indicates the average squared difference between the predicted and actual values

test_mae = mean_absolute_error(y_test, model.predict(X_test))
print(f"Test MAE: {test_mae:.4f}") #evaluates the performance of the regression model on the test data using Mean Absolute Error, which indicates the average absolute difference between the predicted and actual values

test_mse = mean_squared_error(y_test, model.predict(X_test))
print(f"Test MSE: {test_mse:.4f}") #evaluates the performance of the regression model on the test data using Mean Squared Error, which indicates the average squared difference between the predicted and actual values

train_preds = model.predict(X_train)
print(f"Train R²: {r2_score(y_train, train_preds):.4f}") #evaluates the performance of the regression model on the training data using R-squared metric, which indicates how well the model explains the variance in the target variable (market value) based on the features used in the model
...
# Format axes with euro formatting
def euro_format(x, p):
    return f'€{x/1e6:.0f}M'

plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(euro_format))
plt.gca().yaxis.set_major_formatter(plt.FuncFormatter(euro_format))
...
plt.scatter(y_pred, errors, alpha=0.5, edgecolors='k', linewidth=0.5)
